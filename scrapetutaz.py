...
def main():
    ...
    for i in range(1,100):
        url = 'https://tut.az/dasinmaz-emlak/ev-alqi-satqisi/?page='+str(i)
        driver.get(url)
        scrap_link(driver)
...
def scrap_link(driver):
    ...
    regexp0 = re.compile(r'/ev-alqi-satqisi/')
    regexp1 = re.compile(r'-satilir-')
    regexp2 = re.compile(r'-icareye-verilir-')
    #print ("Found the URL:", a['href'])
    for a in results:
        if regexp0.search(a['href']):
            if regexp1.search(a['href']) or regexp2.search(a['href']):
                print ("Found the URL:", a['href'])
                sql = "INSERT IGNORE INTO tb_tutaz_url (siteurl, status) VALUES (%s, %s)"
                val = (a['href'], 0)
